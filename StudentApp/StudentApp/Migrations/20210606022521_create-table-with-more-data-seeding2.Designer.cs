// <auto-generated />
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using StudentApp.Data;

namespace StudentApp.Migrations
{
    [DbContext(typeof(AppDbContext))]
    [Migration("20210606022521_create-table-with-more-data-seeding2")]
    partial class createtablewithmoredataseeding2
    {
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "3.1.10")
                .HasAnnotation("Relational:MaxIdentifierLength", 64);

            modelBuilder.Entity("StudentApp.Models.Course", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    b.Property<string>("Img")
                        .HasColumnType("text");

                    b.Property<string>("Name")
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.ToTable("Courses");

                    b.HasData(
                        new
                        {
                            Id = 1,
                            Img = "https://upload.wikimedia.org/wikipedia/commons/thumb/e/ee/.NET_Core_Logo.svg/1200px-.NET_Core_Logo.svg.png",
                            Name = ".net"
                        },
                        new
                        {
                            Id = 2,
                            Img = "https://ensocore.com/media/61/reactjs-logo-sticker%20%281%29.jpg",
                            Name = "React"
                        },
                        new
                        {
                            Id = 3,
                            Img = "https://bgasparotto.com/wp-content/uploads/2016/11/design-patterns-logo-2.png",
                            Name = "DesignPattern"
                        },
                        new
                        {
                            Id = 4,
                            Img = "https://i0.wp.com/modev.net/wp-content/uploads/2018/06/unit-testing.png?resize=1024%2C545&ssl=1",
                            Name = "UintTest"
                        },
                        new
                        {
                            Id = 5,
                            Img = "https://download.logo.wine/logo/MySQL/MySQL-Logo.wine.png",
                            Name = "MySql"
                        });
                });

            modelBuilder.Entity("StudentApp.Models.Question", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    b.Property<string>("Answer")
                        .HasColumnType("text");

                    b.Property<int>("CourseId")
                        .HasColumnType("int");

                    b.Property<int>("Difficulty")
                        .HasColumnType("int");

                    b.Property<string>("question")
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.HasIndex("CourseId");

                    b.ToTable("Questions");

                    b.HasData(
                        new
                        {
                            Id = 1,
                            Answer = "This is a common way for interviewers to start an interview for a software developer who needs to specialize in the .NET Framework. Show the interviewer your ability to utilize the .NET Framework effectively by describing what it is, when to use it and its main features. Try to show your practical knowledge of the system.",
                            CourseId = 1,
                            Difficulty = 0,
                            question = "What is the .NET Framework?"
                        },
                        new
                        {
                            Id = 2,
                            Answer = "CTS stands for Common Type System. It follows a set of structured rules according to which a data type should be declared and used in the program code. It is used to describe all the data types that are going to be used in the application.",
                            CourseId = 1,
                            Difficulty = 0,
                            question = "What is CTS?"
                        },
                        new
                        {
                            Id = 3,
                            Answer = ".NET is an infrastructure that consists of multiple features like application domain, a common type system and profiling. Isolating one or two important pieces may be a challenge, but the interviewer may be looking for the answers “class library” and “Common Language Runtime.” It’s critical to make a case for why those are the most essential features. This is an opportunity to show your skills go beyond just technical because you can also apply critical thinking.",
                            CourseId = 1,
                            Difficulty = 0,
                            question = "What are the most important aspects of .NET?"
                        },
                        new
                        {
                            Id = 4,
                            Answer = "When developers work within an established framework, they should be aware of certain nuances, especially those between managed and unmanaged code. This question allows you to display your knowledge of working within the framework and to differentiate that knowledge from working in other coding environments.",
                            CourseId = 1,
                            Difficulty = 0,
                            question = "What is the difference between managed and unmanaged code?"
                        },
                        new
                        {
                            Id = 5,
                            Answer = "CLR stands for Common Language Runtime. It is one of the most important components of the .Net framework. It provides building blocks for many applications.",
                            CourseId = 1,
                            Difficulty = 0,
                            question = "What is CLR?"
                        },
                        new
                        {
                            Id = 6,
                            Answer = "MVC stands for Model View Controller. It is an architectural model for building the .Net applications.",
                            CourseId = 1,
                            Difficulty = 0,
                            question = "What is MVC?"
                        },
                        new
                        {
                            Id = 7,
                            Answer = "Exe and DLLs are Assembly executable modules.",
                            CourseId = 1,
                            Difficulty = 0,
                            question = "What is an EXE and a DLL?"
                        });
                });

            modelBuilder.Entity("StudentApp.Models.Student", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    b.Property<string>("Name")
                        .HasColumnType("text");

                    b.Property<string>("img")
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.ToTable("Students");

                    b.HasData(
                        new
                        {
                            Id = 1,
                            Name = "Ahmed",
                            img = "https://previews.123rf.com/images/jemastock/jemastock1710/jemastock171012232/88397537-male-student-carrying-bag-happy-cartoon-icon-image-vector-illustration-design.jpg"
                        },
                        new
                        {
                            Id = 2,
                            Name = "Abdulrahman sarawiq",
                            img = "data:image/jpeg;base64,"
                        },
                        new
                        {
                            Id = 3,
                            Name = "Abdulmajeed Almaymuni",
                            img = "https://previews.123rf.com/images/jemastock/jemastock1710/jemastock171011350/88357846-male-student-carrying-bag-happy-cartoon-icon-image-vector-illustration-design.jpg"
                        },
                        new
                        {
                            Id = 4,
                            Name = "Faisal Alsagri",
                            img = "https://previews.123rf.com/images/jemastock/jemastock1710/jemastock171012232/88397537-male-student-carrying-bag-happy-cartoon-icon-image-vector-illustration-design.jpg"
                        },
                        new
                        {
                            Id = 5,
                            Name = "Ibrahim Alobaysi",
                            img = "https://d2gg9evh47fn9z.cloudfront.net/800px_COLOURBOX36785218.jpg"
                        },
                        new
                        {
                            Id = 6,
                            Name = "Turki Alqurashi",
                            img = "https://eversity.edu/wp-content/uploads/2018/07/malestudent.jpg"
                        },
                        new
                        {
                            Id = 7,
                            Name = "Adel Kalu",
                            img = "https://previews.123rf.com/images/jemastock/jemastock1710/jemastock171012232/88397537-male-student-carrying-bag-happy-cartoon-icon-image-vector-illustration-design.jpg"
                        },
                        new
                        {
                            Id = 8,
                            Name = "Fatimah Alqhtany",
                            img = "https://previews.123rf.com/images/jemastock/jemastock1710/jemastock171011278/88357184-student-carrying-bag-happy-female-cartoon-icon-image-vector-illustration-design.jpg"
                        },
                        new
                        {
                            Id = 9,
                            Name = "Amal Almotairi",
                            img = "https://previews.123rf.com/images/jemastock/jemastock1710/jemastock171012175/88395766-student-carrying-bag-happy-red-hair-female-cartoon-icon-image-vector-illustration-design.jpg"
                        },
                        new
                        {
                            Id = 10,
                            Name = "Arwa Wan La",
                            img = "https://previews.123rf.com/images/jemastock/jemastock1710/jemastock171011278/88357184-student-carrying-bag-happy-female-cartoon-icon-image-vector-illustration-design.jpg"
                        },
                        new
                        {
                            Id = 11,
                            Name = "Albandry AlQaseemi",
                            img = "https://cdn4.vectorstock.com/i/1000x1000/90/68/young-woman-student-cartoon-vector-19619068.jpg"
                        });
                });

            modelBuilder.Entity("StudentApp.Models.StudentCourse", b =>
                {
                    b.Property<int>("StudentId")
                        .HasColumnType("int");

                    b.Property<int>("CourseId")
                        .HasColumnType("int");

                    b.HasKey("StudentId", "CourseId");

                    b.HasIndex("CourseId");

                    b.ToTable("StudentCourse");

                    b.HasData(
                        new
                        {
                            StudentId = 1,
                            CourseId = 1
                        },
                        new
                        {
                            StudentId = 1,
                            CourseId = 2
                        },
                        new
                        {
                            StudentId = 1,
                            CourseId = 3
                        },
                        new
                        {
                            StudentId = 1,
                            CourseId = 4
                        },
                        new
                        {
                            StudentId = 1,
                            CourseId = 5
                        },
                        new
                        {
                            StudentId = 2,
                            CourseId = 2
                        });
                });

            modelBuilder.Entity("StudentApp.Models.Question", b =>
                {
                    b.HasOne("StudentApp.Models.Course", "Course")
                        .WithMany("Questions")
                        .HasForeignKey("CourseId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("StudentApp.Models.StudentCourse", b =>
                {
                    b.HasOne("StudentApp.Models.Course", "Course")
                        .WithMany("StudentCourse")
                        .HasForeignKey("CourseId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("StudentApp.Models.Student", "Student")
                        .WithMany("StudentCourse")
                        .HasForeignKey("StudentId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });
#pragma warning restore 612, 618
        }
    }
}
